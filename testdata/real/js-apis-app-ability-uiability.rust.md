...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3c/v3/YeTMOZs-R1Wv_Ys4x4UYNg/zh-cn_image_0000002693205804.png?HW-CC-KV=V1&HW-CC-Date=20260828T005351Z&HW-CC-Expire=86400&HW-CC-Sign=52E7E2C217A19BE6F54E7A4F5667A5396224F4BAAD79535C948CF63239EC0B47)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5e/v3/CatjwXRDQdWrJXoxI28CsQ/zh-cn_image_0000002693043438.png?HW-CC-KV=V1&HW-CC-Date=20260828T005351Z&HW-CC-Expire=86400&HW-CC-Sign=EB0BC1A2F61B46909373E93487E2EFECF1D339736449671C5732CD6AE4178340)
...
| launchWant | Want | 否 | 否 | UIAbility冷启动时接收到的Want参数，取值为onCreate接收到的Want参数。 元服务API：从API version 11开始，该接口支持在元服务中使用。 |
...
> [!NOTE]
> 在应用迁移启动时，无论是 冷启动 还是 热启动 ，都会在执行完 onCreate() / onNewWant() 后，触发onWindowStageRestore()生命周期函数，不执行onWindowStageCreate()生命周期函数。
...
> [!NOTE]
> 在执行完onDestroy生命周期回调后，应用可能会退出，从而导致其中的异步函数（比如异步写入数据库）未能正确执行。在此情况下，推荐使用Promise异步回调。 该回调仅在UIAbility正常退出时触发，当UIAbility异常退出（例如低内存终止进程）时，该回调将不被触发。
...
> [!NOTE]
> 从API version 20开始，当 onSaveStateAsync 实现时，本回调函数将不执行。
...
| wantParam | Record<string, Object> | 是 | 用户自定义的应用状态数据，应用再启动时被保存在onCreate的Want.parameters中。 |
...
> [!NOTE]
> 从API version 15开始，当 UIAbility.onPrepareToTerminateAsync 实现时，本回调函数将不执行。当 AbilityStage.onPrepareTerminationAsync 或 AbilityStage.onPrepareTermination 实现时，在dock栏或系统托盘处右键点击关闭，本回调函数将不执行。 如果应用本身或者所使用的三方框架注册了 window.WindowStage.on('windowStageClose') 监听，本回调函数将不执行。
...
> [!NOTE]
> 当 AbilityStage.onPrepareTerminationAsync 或 AbilityStage.onPrepareTermination 实现时，在dock栏或系统托盘处右键点击关闭，本回调函数将不执行。 如果应用本身或者所使用的三方框架注册了 window.WindowStage.on('windowStageClose') 监听，本回调函数将不执行。 若异步回调内发生crash，按超时处理，执行等待超过10秒未响应，UIAbility将被强制关闭。
...
> [!NOTE]
> 该生命周期回调不支持 specified启动模式 。 通过 startAbility 等方法拉起协同方应用时，需要在Want对象中设置协同标记 Flags 为FLAG_ABILITY_ON_COLLABORATE。 冷启动 时，该回调在 onForeground 前或 onBackground 后调用； 热启动 时，该回调在 onNewWant 前调用。
...
| --- | --- | --- | --- |
| method | string | 是 | 由Caller和Callee双方约定好的方法名，Callee方通过该字段区分消息类型。 |
| callback | CalleeCallback | 是 | 一个rpc.MessageSequence类型入参的js通知同步回调函数， 回调函数至少要返回一个空的rpc.Parcelable数据对象， 其他视为函数执行错误。 |
